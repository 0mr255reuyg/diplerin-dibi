...
def puan(tke,sk,sd,mfi,rsi,mfi_dip=35.0):
    ...
    so=np.nan if (sk is None or sd is None or np.isnan(sk) or np.isnan(sd)) else (sk+sd)/2
    p={"TKE":_p(tke,20,50,30),"StochRSI":_p(so,20,50,30),
       "MFI":_p(mfi,mfi_dip,55,30),"RSI":_p(rsi,40,60,10)}
    p["TOPLAM"]=round(sum(p.values()),2)
    return p

def analiz(hisse,df):
    try:
        tke=hesapla_tke(df); sk,sd=hesapla_stoch_rsi(df)
        mfi=hesapla_mfi(df); rsi=hesapla_rsi(df)
        md=mfi_dip_bul(mfi)
        ...
        p=puan(tv,skv,sdv,mv,rv,md)
        return {"Hisse":hisse,"Fiyat":round(fv,2),"TKE":round(tv,2),
                "StochRSI_K":round(skv,2),"StochRSI_D":round(sdv,2),
...
def pl_detay(s):
    fig=make_subplots(rows=5,cols=1,shared_xaxes=True,row_heights=[.30,.18,.18,.18,.16],
        vertical_spacing=0.03,
        subplot_titles=[s["Hisse"]+" – Fiyat (TL)","TKE (30p)","Stokastik RSI (30p)","MFI (30p)","RSI (10p)"])
    def ln(sr,nm,cl,rw,ds="solid"):
        fig.add_trace(go.Scatter(x=sr.index,y=sr.values,name=nm,line=dict(color=cl,width=2,dash=ds)),row=rw,col=1)
    def hl(rw,y,cl,lb):
    ...
    ln(s["_sk"],"%K",R["StochRSI"],3); ln(s["_sd"],"%D","#FF9F1C",3,ds="dot"); hl(3,20,R["g"],"Dip(20)")
    ln(s["_mfi"],"MFI",R["MFI"],4)
    hl(4,s["MFI_Dip"],R["g"],"Dip("+str(round(s["MFI_Dip"],1))+")"); hl(4,80,R["r"],"Tepe(80)")
    ln(s["_rsi"],"RSI",R["RSI"],5); hl(5,40,R["g"],"Dip(40)"); hl(5,60,R["r"],"Tepe(60)")
    fig.update_layout(template="plotly_dark",height=750,paper_bgcolor=R["bg"],plot_bgcolor=R["panel"],
        font=dict(color=R["txt"],family="monospace"),legend=dict(orientation="h",y=-0.04),
        margin=dict(l=50,r=80,t=50,b=40),
        title=dict(text="📊 "+s["Hisse"]+" – İndikatör Analizi",font=dict(size=17,color=R["fiyat"])))
    return fig
...
def pl_radar(s):
    ...
    fig.update_layout(polar=dict(bgcolor=R["panel"],
        ...
        angularaxis=dict(color=R["txt"])),
        paper_bgcolor=R["bg"],font=dict(color=R["txt"]),showlegend=False,height=340,
...
def pl_bar(df,top_n=20):
    ...
    fig=go.Figure()
    for k,cl,lb in [("Puan_TKE",R["TKE"],"TKE (30p)"),("Puan_StochRSI",R["StochRSI"],"StochRSI (30p)"),
                    ("Puan_MFI",R["MFI"],"MFI (30p)"),("Puan_RSI",R["RSI"],"RSI (10p)")]:
        fig.add_trace(go.Bar(y=top["Hisse"],x=top[k],name=lb,orientation="h",marker=dict(color=cl,opacity=0.85)))
    fig.update_layout(barmode="stack",template="plotly_dark",paper_bgcolor=R["bg"],plot_bgcolor=R["panel"],
        font=dict(color=R["txt"],family="monospace"),xaxis=dict(title="Toplam Puan",range=[0,100]),
        title=dict(text="🏆 En Yüksek Puanlı "+str(top_n)+" Hisse",font=dict(size=16,color=R["fiyat"])),
        legend=dict(orientation="h",y=-0.12),height=max(400,top_n*28),margin=dict(l=70,r=30,t=60,b=80))
    return fig
...
def pl_heat(df,top_n=30):
    ...
    fig=go.Figure(go.Heatmap(z=z,x=etk,y=top["Hisse"].tolist(),colorscale="RdYlGn",zmin=0,zmax=100,
        text=[[str(round(v,1)) for v in row] for row in z],texttemplate="%{text}",
        textfont=dict(size=10),colorbar=dict(title="Puan")))
    fig.update_layout(template="plotly_dark",paper_bgcolor=R["bg"],plot_bgcolor=R["panel"],
        ...
        title=dict(text="🌡️ Isı Haritası – İlk "+str(top_n)+" Hisse",font=dict(size=16,color=R["fiyat"])))
    return fig
...
def mpl_detay(s):
    ...
    axs[0].set_title(s["Hisse"]+" – Toplam: "+str(s["Puan_Toplam"])+"/100",color=R["fiyat"],fontsize=12)
    for ax,sr,cl,lb,dp,tp in [(axs[1],s["_tke"],R["TKE"],"TKE",20,80),
...
def mpl_puan(s):
    ...
    for b,p in zip(bars,ps):
        ax.text(b.get_x()+b.get_width()/2,b.get_height()+0.3,str(round(p,1)),
                ha="center",va="bottom",color=R["txt"],fontsize=9)
    ax.set_ylim(0,35); ax.tick_params(colors=R["txt"]); ax.spines[:].set_color("#333355")
    ax.set_title(s["Hisse"]+" – Toplam: "+str(s["Puan_Toplam"])+"/100",color=R["fiyat"],fontsize=10)
    fig.tight_layout(); return fig

# ── STREAMLIT ────────────────────────────────────────
st.set_page_config(page_title="BIST 100 Dip Bulucu",page_icon="📉",layout="wide",initial_sidebar_state="expanded")
st.markdown("""
<style>
...
.yesil{color:#06D6A0;}.sari{color:#FFE66D;}.turuncu{color:#FF9F1C;}.kirmizi{color:#EF476F;}
.stButton>button{background:linear-gradient(135deg,#FF6B35,#FF9F1C);color:white;font-weight:700;border:none;border-radius:8px;padding:.5rem 1.5rem;}
...
with st.sidebar:
    ...
    min_puan=st.slider("Minimum Puan",0,100,40,step=5)
    top_n=st.slider("Gösterilecek Hisse",5,50,20,step=5)
    st.divider()
    st.caption("TKE:21 | StochRSI:21/21 | MFI:21 | RSI:21")
    st.caption("TKE 30p · StochRSI 30p · MFI 30p · RSI 10p")
    st.divider()
...
def goster(s):
    ...
    for col,(lb,val,mx) in zip([c1,c2,c3,c4,c5],[
        ...
        r="yesil" if val>=mx*0.7 else "sari" if val>=mx*0.4 else "kirmizi"
        col.markdown('<div class="kart"><div class="kart-val '+r+'">'+str(round(val,1))+
            '</div><div class="kart-lbl">'+lb+' / '+str(mx)+'</div></div>',unsafe_allow_html=True)
    st.markdown("")
    ...
    with st.expander("🖨️ PNG İndir"):
        ...
        st.image(buf)
        st.download_button("⬇️ PNG İndir",data=buf,file_name=s["Hisse"]+"_analiz.png",mime="image/png")
...
if tara_btn:
    ...
    for i,h in enumerate(BIST100):
        ...
        prog.progress((i+1)/top); txt.text("⏳ "+h+" ("+str(i+1)+"/"+str(top)+")")
        time.sleep(0.04)
    ...
    st.session_state.df_sonuc=df_s; st.session_state.sonuclar=sonuclar
    st.success("✅ "+str(len(df_s))+" hisse analiz edildi!")

if st.session_state.df_sonuc is not None:
    df=st.session_state.df_sonuc; df_f=df[df["Puan_Toplam"]>=min_puan]
    st.markdown("## 🏆 Dip Puanlama Sonuçları")
    c1,c2,c3,c4=st.columns(4)
    t1=df.iloc[0]
    for col,val,lb in [(c1,str(len(df_f)),"Filtreden Geçen"),
        (c2,str(t1["Hisse"]),"En İyi: "+str(t1["Puan_Toplam"])+"p"),
        (c3,str(round(df["Puan_Toplam"].mean(),1)),"Ortalama Puan"),
        (c4,str((df["Puan_Toplam"]>=50).sum()),"50+ Puan")]:
    ...
    with t3:
        kols=["Hisse","Fiyat","Puan_Toplam","TKE","Puan_TKE","StochRSI_K","StochRSI_D",
              "Puan_StochRSI","MFI","MFI_Dip","Puan_MFI","RSI","Puan_RSI"]
        st.dataframe(df_f[kols].style.background_gradient(subset=["Puan_Toplam"],cmap="RdYlGn").format(precision=2),
            use_container_width=True,height=500)
        csv=df_f[kols].to_csv(index=False).encode("utf-8")
        st.download_button("⬇️ CSV İndir",data=csv,file_name="bist100_dip.csv",mime="text/csv")
...
if tek_btn:
    ...
    if df_t is None: st.error("❌ "+tek_hisse+" için veri alınamadı.")
    else:
...
st.divider()
st.markdown("<div style='text-align:center;color:#444;font-size:.75rem;'>BIST 100 Dip Bulucu · TKE (Kıvanç Özbilgiç) · Stokastik RSI · MFI · RSI<br>⚠️ Yatırım tavsiyesi değildir. Veriler Yahoo Finance'den çekilmektedir.</div>",unsafe_allow_html=True)
